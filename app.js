...
document.addEventListener('DOMContentLoaded', async function() {
    try {
        ...
        const lastMonthValue = lastMonth.split(' ');
        const lastMonthFormatted = `${lastMonthValue[1]}-${lastMonthValue[0]}`;
...
function updateDashboard() {
    ...
    const currentMonth = historicalData.labels.find(label => {
        const [month, year] = label.split(' ');
        return `${year}-${month.toLowerCase()}` === periodo;
    }) || historicalData.labels[historicalData.labels.length - 1];
    
    const currentMonthIndex = historicalData.labels.indexOf(currentMonth);
    
    // Filtrar KPIs según perspectiva seleccionada
    let filteredKpis = isMatrizView ? kpisData.filter(kpi => kpi.granularidad.includes("Central")) : kpisData;
    
    if (perspectiva !== 'todas') {
        filteredKpis = filteredKpis.filter(kpi => kpi.perspectiva === perspectiva);
    }
    
    // Ajustar valores según sucursal/oficial seleccionado
    if (!isMatrizView && (sucursalId !== 'todas' || oficial !== 'todos')) {
    ...
    // Obtener valores históricos para el mes seleccionado
    const eficienciaValue = historicalData.eficiencia[currentMonthIndex];
    const calidadValue = historicalData.calidad[currentMonthIndex];
    const experienciaValue = historicalData.experiencia[currentMonthIndex];
    ...
    Object.keys(groupedData).forEach(perspectiva => {
        ...
        Object.keys(groupedData[perspectiva]).forEach(proceso => {
            ...
            groupedData[perspectiva][proceso].forEach(kpi => {
                // Obtener el valor histórico para este KPI en el mes seleccionado
                const historicalValue = historicalValues[kpi.id]?.[currentMonth] || kpi.valorActual;
                
                const cumplimiento = formatToTwoDecimals(
                    kpi.unidad.includes("Horas") || kpi.unidad.includes("Minutos") || kpi.nombre.includes("Errores")
                    ? (kpi.valorBudget / historicalValue) * 100
                    : (historicalValue / kpi.valorBudget) * 100
                );
                ...
                let valorActualFormatted = formatValue(historicalValue, kpi.unidad);
